...
// Utilisation CLI: php create_admin.php "Nom" "email@local" "MotDePasse"
...
function upsertAdmin(mysqli $mysqli, string $nom, string $email, string $plainPassword): array {
...
if (PHP_SAPI === 'cli') {
    ...
    if ($plainPassword === '') {
        echo "Usage: php create_admin.php \"Nom\" \"email@local\" \"MotDePasse\"\n";
        exit(1);
    }

    [$ok, $message] = upsertAdmin($mysqli, $nom, $email, $plainPassword);
    echo $message . "\n";
    exit($ok ? 0 : 1);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    [$success, $message] = upsertAdmin($mysqli, $nom, $email, $plainPassword);
}
?>
<!doctype html>
<html lang="fr">
