...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_session') {
        ...
        if (!$errors) {
            header('Location: /club-lecture/pages/admin/sessions.php?book_id=' . $bookId);
            exit;
        }
    }
...
<head>
  ...
  <title>Admin - Gestion des sessions</title>
...
<body>
  ...
  <main>
    ...
    <section>
      ...
      <?php if (!$bookIdFilter): ?>
        <p>Choisis un livre pour voir ses sessions.</p>
      <?php elseif (!$sessions): ?>
        <p>Aucune session pour ce livre.</p>
      <?php else: ?>
        <ul>
          <?php foreach ($sessions as $session): ?>
            <li>
              ...
              <br>
              <a href="/club-lecture/pages/admin/sessions.php?book_id=<?= (int) $bookIdFilter ?>&edit=<?= (int) $session['id'] ?>">Modifier</a>

              <form method="post" class="action-inline-form" onsubmit="return confirm('Supprimer cette session ?');">
                <?= csrfInput() ?>
    ...
    <section>
      ...
      <?php if (!$bookIdFilter): ?>
        <p>Choisis un livre pour voir les inscriptions.</p>
      <?php elseif (!$attendanceRows): ?>
        <p>Aucune inscription pour ce livre.</p>
      <?php else: ?>
